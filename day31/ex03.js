...
let listTarotGuess = JSON.parse(localStorage.getItem('tarot')) || []
function saveTarot() {
  localStorage.setItem('tarot', JSON.stringify(listTarotGuess))
}
function createTarot(data) {
    ...
    button.addEventListener('click', function(e) {
        ...
        const random_guess = Math.floor(Math.random()*78)
        h2.textContent = data[random_guess].name
        p.textContent = data[random_guess].description

        guess.appendChild(h2)
        guess.appendChild(p)

        listTarotGuess.push({name: data[random_guess].name, description: data[random_guess].description})
        saveTarot()
        createListTarotCard()
    })
    saveTarot()
...
function createListTarotCard() {
  listCard.textContent = ''

  const title = document.createElement('h2')
  title.textContent = 'Danh sách các lá bài Tarot đã rút'
